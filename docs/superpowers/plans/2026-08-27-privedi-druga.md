...
- [x] **Step 2: «Реферал — отменить приглашение»**
...
- [x] **Step 3: Проверить под временным пользователем**
...
- [x] **Step 4: Записать id флоу в спеку, коммит**
...
- [x] **Step 2: Проверить на живых данных**
...
- [x] **Step 1: Нормализация номера — чистой функцией**
...
- [x] **Step 4: Пункт в профиле, маршрут, строки**
...
- [x] **Step 3: Убрать за собой и свериться**
...
Сквозной путь пройден целиком на временных записях, с заглушённой рассылкой пушей:

| Шаг | Результат |
|---|---|
| курьер приглашает через флоу | `{ok:true}`, строка `invited` с его id из авторизации |
| курьер читает свой список | видит только своё приглашение |
| друг регистрируется по номеру | строка → `registered`, `invitee_id` проставлен |
| друг довозит первый заказ | баланс `0 → 10`, строка `paid` с суммой и датой, уведомление нужному человеку |

Отдельно проверено: вторая доставка того же друга и повторное завершение того же заказа
выплату **не повторяют**.

После прогона база вернулась к эталону: 32 курьера, 186 жетонов, ноль тестовых записей,
адрес сервиса пушей восстановлен.

Не подтверждён отдельной строкой случай «доставку завершает никем не приглашённый курьер» —
проверка оборвалась на нём. Он покрыт случаем со второй доставкой: там строка уже оплачена,
и выплаты не происходит по той же ветке кода.
