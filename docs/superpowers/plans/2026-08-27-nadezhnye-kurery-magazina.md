...
⚠️ Помимо самого поля пришлось добавить `trusted_only` в `leanFields` запроса `getOrders` — иначе значение просто не приезжает с сервера и значок никогда не показывается.
...
- [x] **Step 3 (необязательный): Уведомление о добавлении в список** — сделано

Флоу «Уведомление - Надёжный курьер» `8cde3e47-84dc-440d-b766-1487ec16810a`, событие `items.create` на `shop_trusted_couriers`: `read_row → build → create_notif → send_push → request_push`. Тип уведомления `trusted_added`, иконка добавлена в `notifTypeIcon`.

Проверено живьём с заглушённой отправкой: одно уведомление нужному курьеру, название магазина подставляется (организация, иначе имя с фамилией). Тестовые записи удалены.
...
### Task 8: Фото курьера магазину — ОТКЛОНЕНО

**Решение пользователя от 27.08: фото не делаем, оставляем имя и телефон.**

Изменений в коде не потребовалось: строка «Курьер — Имя Фамилия» с номером и
кнопкой звонка магазину уже показывается в `order_details_section.dart:198`.
Задача закрыта без правок.

Причина отказа — цена вопроса. Селфи курьера пришлось бы отдавать по открытой
ссылке `$baseUrl/assets/<id>` без авторизации (так грузятся фото заказов,
`order_images_section.dart:57`), то есть документ человека стал бы доступен
любому, кто знает id файла. Закрывать файлы правами ради одного аватара —
значит трогать и загрузку фото заказов, а это риск, несоразмерный выгоде.

Побочное следствие: `OrderDto.courierSelfieFileId` разбирается, но нигде не
используется. Поле оставлено как есть — оно ничего не стоит и может
понадобиться, если решение когда-нибудь пересмотрят.
