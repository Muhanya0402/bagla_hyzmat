...
class CancelReasonModal extends StatefulWidget {
  ...
  /// либо действие удалось, либо повторять его уже бессмысленно (например,
  /// заказ стал терминальным, пока форма была открыта). Показ ошибок и
  /// обновление списка остаются на вызывающей стороне: у отмены магазином
  /// и отказа курьера они разные.
