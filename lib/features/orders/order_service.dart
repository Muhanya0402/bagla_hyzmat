...
class OrderService {
  ...
  Future<bool> createOrder({
    ...
    // уведомление успело бы уйти всем курьерам.
  ...
  /// Нужно ли отдавать этот заказ только надёжным курьерам магазина.
  ///
  /// Режим включается сам, по сумме: магазин не должен помнить про галочку,
  /// когда отправляет золото или технику.
  ///
  /// ⚠️ **Условие о непустом списке обязательно.** Заказ «только для своих»
  /// не открывается всем никогда — так решено сознательно. Если бы режим
  /// включался у магазина с пустым списком, дорогой заказ стал бы невидим
  /// вообще для всех и навсегда, причём магазин ничего для этого не нажимал
  /// и не понял бы, почему тишина. Нет надёжных — заказ уходит всем, а
  /// магазину показывается подсказка.
  ///
  /// [threshold] равный нулю выключает режим целиком — это рубильник в
  /// настройках Directus, позволяющий включить функцию без пересборки.
